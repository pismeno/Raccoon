...
void IRGenerator::visit(CallExpr &node) {
    ...
    } else if (node.func == "raccoon_gc_collect") {
        llvm::FunctionCallee gcFn = module->getOrInsertFunction(
            "raccoon_gc_collect",
            llvm::FunctionType::get(builder->getVoidTy(), {}, false)
        );
        this->lastValue = builder->CreateCall(gcFn);
    } else if (node.func == "raccoon_push_root") {
        if (node.args.size() != 1) throw CompileError("raccoon_push_root expects 1 argument.");

        auto* varNode = dynamic_cast<VariableExpr*>(node.args[0].get());
        if (!varNode) throw CompileError("raccoon_push_root expects a variable argument to access its address.");

        std::optional<VarInfo> varInfo = varTable.lookup(varNode->name);
        if (!varInfo) throw CompileError("Undefined variable: " + varNode->name);

        llvm::FunctionCallee pushFn = module->getOrInsertFunction(
            "raccoon_push_root",
            llvm::FunctionType::get(builder->getVoidTy(), {builder->getPtrTy()}, false)
        );
        this->lastValue = builder->CreateCall(pushFn, {varInfo->address});
    } else if (node.func == "raccoon_pop_roots") {
        if (node.args.size() != 1) throw CompileError("raccoon_pop_roots expects 1 argument.");

        llvm::FunctionCallee popFn = module->getOrInsertFunction(
            "raccoon_pop_roots",
            llvm::FunctionType::get(builder->getVoidTy(), {builder->getInt64Ty()}, false)
        );

        node.args[0]->accept(*this);
        this->lastValue = builder->CreateCall(popFn, {this->lastValue});
    } else {
...
void IRGenerator::visit(ObjectDecl &node) {
    ...
    if (isGlobal) {
        ...
        if (!constInit) throw CompileError("Global object initializer must be a constant.");
        auto* globalVar = new llvm::GlobalVariable(
...
void IRGenerator::visit(MemberExpr &node) {
    ...
    auto structIt = structMap.find(objectType->classVariableName);
    if (structIt == structMap.end()) {
        throw CompileError("Struct type not found: " + objectType->classVariableName);
    }
    llvm::StructType* structType = structIt->second;

    llvm::Value* memberPtr = builder->CreateStructGEP(
        structType, objPtr, fieldIndex, node.member + "_ptr"
    );
    llvm::Type* memberType = structType->getElementType(fieldIndex);
    this->lastValue = builder->CreateLoad(memberType, memberPtr, node.member + "_val");
}
